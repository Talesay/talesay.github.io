    ...
    window.EntitySkeleton01 = ig.Entity.extend({
        ...
        handleOffscreen: function () {
            if (ig.game.player.pos.x < this.pos.x - 196) {
                return;
            } else if (ig.game.player.pos.x > this.pos.x + 196) {
        ...
        collideWith: function (other, axis) {
            if (other.attacked && other.type === ig.Entity.TYPE.A) {
                other.crit();
                this.kill(true);
            } else if (other.jumped && other.type === ig.Entity.TYPE.A) {
                other.hitted = true;
                other.health -= 1;
                this.kill(false);
            } else if (other.type === ig.Entity.TYPE.A) {
                other.hitted = true;
                other.health -= 1;
                this.kill(false);
            }
